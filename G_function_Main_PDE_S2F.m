function [result] = G_function_Main_PDE_S2F(rho1,lambda1,mu1,sigma1,gamma1)
...
%% STEP 1: Parameters
% A. Preferences
rho    = rho1;% 0.1; %0.1;   % impatience rate in discount factor e^(-rho*t)
lambda = lambda1; %2/3;   % the weight of agent 1(more RRA, RRA=1) in the RA utility function
...
mu      = mu1; %0.05; %E[dY/Y]
sigma   = sigma1; %0.3;  %Volatility term of dY/Y
                                             
%% STEP 2: Discretization 
% A. State space: structured grid
Ymax = 100;   
    % This Ymin is consistent with gamma1 (problem: Y would be different when we change gamma1)
    %Ymin = ( ( 1 + ( b/2 )/( 1 + (sqrt(b)/2)^(gamma1) )^(1/gamma1) )^2 - 1 )/b;  %consistent with m0=1
    
    % I keep constant Y for comparison when gamma1 changes
    Ymin = ( ( 1 + ( b/2 )/( 1 + (sqrt(b)/2) ) )^2 - 1 )/b;  %consistent with m0=1
...
%column vectors
R1 = [c1' c2' S W1' W2' ]; 
R2 = [w11' w12' w21' w22' N11' N21' NB1' NB2'];
R3 = [S pd r' psi' beta' m' sigmat'];
